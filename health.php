    ...
    return [
        ...
        [
            ...
            'fix' => [
                ...
                // Existing days become their single site entry, so the reports
                // show history instead of starting empty.
                "INSERT INTO attendance_site_entries
                 ...
                 SELECT da.id, da.employee_id, da.attendance_date,
                        ...
                        da.work_site, da.in_time, da.out_time,
                        GREATEST(0, ROUND(
                            ...
                            - COALESCE((SELECT SUM(TIME_TO_SEC(TIMEDIFF(al.end_time, al.start_time))) / 3600
                                        ...
                                        WHERE al.daily_attendance_id = da.id
                                          ...
                                          AND al.end_time IS NOT NULL), 0), 2)),
                        'migration'
                 FROM daily_attendance da
                 WHERE da.in_time IS NOT NULL
                   AND NOT EXISTS (SELECT 1 FROM attendance_site_entries e
                                   WHERE e.daily_attendance_id = da.id)",
            ],
            'why' => 'Multi-site attendance: work at each site per day, written by the entry screen.',
        ],
        [
