...
3. Open `https://your-site/sql/db.php` and fill in the database details, **or**
   run `php sql/db.php` from a cPanel Terminal / cron job.
4. When it reports success, **delete `sql/db.php` and `sql/db.config.json`** from
   the server.

After a successful import the installer writes `sql/.installed` and refuses every
further web request, so it cannot be replayed by a visitor.
